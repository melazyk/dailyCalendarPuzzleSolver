...
def recursiveSolve(
    ...
    if len(pieces):
        ...
        for piece in pieces:
            for origin in range(len(piece)):
                ...
                for trans in relTrans:
                    if (
                        (side == "front" and trans.isFront())
                        or (side == "back" and trans.isBack())
                        or side == "both"
                    ):
                        ...
                        if newBoard is not None and not stop:
                            ...
                            tries, nbPcsPut, nbSol, stop, solutions = recursiveSolve(
                                newBoard,
                                newPieces,
                                pid,
                                tries,
        ...
        if printSol:
            print(
                "\nSolution found by process {} in {} after testing {} combinations and putting {} pieces:".format(
                    pid, str(datetime.now() - startTime)[:-7], tries, nbPcsPut
                )
            )
...
def pieceSolve(board, pieces, piece, side, startTime, findAll, printSol, pid, pipe):
    ...
    pipe.send(
        {"tries": tries, "nbSol": nbSol, "nbPcsPut": nbPcsPut, "solutions": solutions}
    )
    print(
        "End of process {} after {} with {} sol. found using {} tries and putting {} pieces".format(
            pid, str(datetime.now() - startTime)[:-7], nbSol, tries, nbPcsPut
        )
    )
